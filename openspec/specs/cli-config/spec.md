...
## Purpose
Provide a user-friendly CLI interface for viewing and modifying global OfficeSpec configuration settings without manually editing JSON files.
## Requirements
...
- **WHEN** `openspec config profile` exits with `No config changes.` inside an OfficeSpec project
...
- **AND** current directory is an OfficeSpec project
