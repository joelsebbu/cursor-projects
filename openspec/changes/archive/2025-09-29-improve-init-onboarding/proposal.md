## Why
The current `openspec init` flow assumes a single assistant selection and stops once an OfficeSpec structure already exists. That makes onboarding feel rigid: teams cannot configure multiple tools in one pass, they do not learn which files were refreshed, and the success copy always references Claude even when other assistants are involved.
...
- Align generated AI-instruction content and specs so CLAUDE.md and AGENTS.md share the same OfficeSpec guidance.
