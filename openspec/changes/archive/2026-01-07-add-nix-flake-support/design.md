...
OfficeSpec is a TypeScript CLI tool using pnpm for dependency management. The project requires Node.js ≥20.19.0. Nix uses its own build system that needs to understand how to fetch dependencies and build the project reproducibly.
...
- Enable OfficeSpec to be run directly via `nix run github:Fission-AI/OpenSpec`
...
**Decision**: Package OfficeSpec using `stdenv.mkDerivation` with pnpm hooks.
