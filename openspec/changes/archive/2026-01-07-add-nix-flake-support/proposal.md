...
OfficeSpec users on NixOS or using the Nix package manager cannot easily install or run OfficeSpec without going through npm. Adding a Nix flake makes OfficeSpec a first-class citizen in the Nix ecosystem, enabling users to run `nix run github:Fission-AI/OpenSpec -- init` or include OfficeSpec in their development environments declaratively.
...
### New Capabilities
- `nix-flake-support`: Nix flake configuration for building and running OfficeSpec
