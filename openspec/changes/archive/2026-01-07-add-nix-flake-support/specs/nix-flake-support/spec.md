...
The system SHALL provide a Nix flake that builds OfficeSpec for multiple platforms.

#### Scenario: Build on Linux x86_64
- **WHEN** user runs `nix build` on x86_64-linux system
- **THEN** system builds OfficeSpec package successfully
...
The system SHALL allow users to run OfficeSpec directly from GitHub without installing.

#### Scenario: Run init command from GitHub
- **WHEN** user runs `nix run github:Fission-AI/OpenSpec -- init`
- **THEN** system downloads and builds OfficeSpec
- **AND** executes `openspec init` command

#### Scenario: Run any OfficeSpec command
...
The system SHALL use pnpm for building OfficeSpec in the Nix flake.
...
- **WHEN** user runs `nix develop` in OfficeSpec repository
