...
OfficeSpec SHALL provide a guided setup flow for users starting workspace planning.

#### Scenario: Creating a workspace through setup
- **WHEN** a user runs `openspec workspace setup`
- **THEN** OfficeSpec SHALL guide the user through creating an OfficeSpec workspace
...
- **THEN** OfficeSpec SHALL ask for the workspace name before asking for repos or folders
...
- **THEN** OfficeSpec SHALL explain that workspace names must be kebab-case
...
- **THEN** OfficeSpec SHALL infer the link name from the folder basename
...
- **THEN** OfficeSpec SHALL show the conflicting link name and the existing path for that link
- **AND** it SHALL ask the user for a different link name before continuing

#### Scenario: Preserving folder-style link names
- **WHEN** OfficeSpec accepts a workspace link name
...
- **THEN** OfficeSpec SHALL let the user add another repo or folder with a simple repeated prompt
...
- **THEN** OfficeSpec SHALL verify that the path resolves to an existing folder
...
- **THEN** OfficeSpec SHALL treat the full value as the path
...
- **THEN** OfficeSpec SHALL create the workspace without prompts
...
- **THEN** OfficeSpec SHALL fail with a clear duplicate link-name error
...
- **THEN** OfficeSpec SHALL fail with a clear message
- **AND** it SHALL explain which flags are required

#### Scenario: Finishing setup
- **WHEN** setup finishes
- **THEN** OfficeSpec SHALL show the workspace location, planning path, and linked repos or folders
...
- **THEN** OfficeSpec SHALL record it in the local workspace registry
...
- **THEN** OfficeSpec SHALL explain that the workspace already exists
- **AND** it SHALL not overwrite the existing workspace

### Requirement: Workspace Discovery
OfficeSpec SHALL let users see the OfficeSpec-managed workspaces available on the current machine.

#### Scenario: Listing workspaces
- **WHEN** a user runs `openspec workspace list`
- **THEN** OfficeSpec SHALL list known managed workspaces
...
- **THEN** OfficeSpec SHALL behave the same as `openspec workspace list`

#### Scenario: Listing when no workspaces exist
- **WHEN** a user runs `openspec workspace list`
- **AND** no managed workspaces exist
- **THEN** OfficeSpec SHALL say that no workspaces were found
...
- **THEN** OfficeSpec SHALL use the current workspace
...
- **THEN** OfficeSpec SHALL record the workspace name and location in the local workspace registry
...
- **THEN** OfficeSpec SHALL report the non-fatal registry warning
...
- **THEN** OfficeSpec SHALL show a workspace picker
...
- **THEN** OfficeSpec SHALL fail without showing a picker
...
- **THEN** OfficeSpec SHALL fail without showing a picker regardless of interactive mode
- **AND** it SHALL print `No known OfficeSpec workspaces. Run 'openspec workspace setup' first.`
...
OfficeSpec SHALL let users link existing repos or folders to a workspace before creating a change.
...
- **THEN** OfficeSpec SHALL use the explicit link name for planning
- **AND** it SHALL store the verified absolute local path as machine-local state
...
- **AND** OfficeSpec SHALL reject missing paths with a clear message

#### Scenario: Resolving linked paths before storage
- **WHEN** a user links a repo or folder path
- **THEN** OfficeSpec SHALL store the verified absolute path for the current runtime
- **AND** relative inputs SHALL be resolved against the command's current working directory
- **AND** OfficeSpec SHALL not translate paths between native Windows, WSL2, and Unix runtimes

#### Scenario: Linking a monorepo folder
- **WHEN** a user links a package, service, app, or directory inside a monorepo
- **THEN** OfficeSpec SHALL store it as a workspace link
- **AND** it SHALL not require that folder to have its own repo-local `openspec/` directory

#### Scenario: Linking without repo-local OfficeSpec
- **WHEN** a user links a path that does not contain repo-local OfficeSpec state
- **THEN** OfficeSpec SHALL keep that repo or folder available for workspace planning
- **AND** it SHALL not treat missing repo-local OfficeSpec state as a link failure

#### Scenario: Link records only
- **WHEN** a user links a repo or folder
- **THEN** OfficeSpec SHALL record workspace state and local path state
...
- **THEN** OfficeSpec SHALL fail with status code `workspace_local_state_invalid`
...
- **THEN** OfficeSpec SHALL explain that the link name is already in use by another link
...
OfficeSpec SHALL let users update existing link paths without recreating the workspace.

#### Scenario: Updating a local path
- **GIVEN** a workspace has a link
- **WHEN** a user runs `openspec workspace relink <name> <path>`
- **THEN** OfficeSpec SHALL keep the stable link name
...
- **THEN** OfficeSpec SHALL explain that the link name is unknown
...
- **THEN** OfficeSpec SHALL not ask for owner or handoff metadata
- **AND** link maintenance SHALL focus on names and local paths

### Requirement: Workspace Health Check
OfficeSpec SHALL explain what the current machine can resolve for a workspace.

#### Scenario: Doctor checks one selected workspace
- **WHEN** a user runs `openspec workspace doctor`
- **THEN** OfficeSpec SHALL inspect one selected workspace
...
- **THEN** OfficeSpec SHALL inspect the current workspace

#### Scenario: Checking a healthy workspace
- **WHEN** a user runs `openspec workspace doctor`
- **THEN** OfficeSpec SHALL show the workspace location and workspace planning path
...
- **WHEN** a user runs `openspec workspace doctor`
- **THEN** OfficeSpec SHALL report a selected-workspace status error
...
- **THEN** OfficeSpec SHALL report status code `workspace_local_state_invalid`
...
OfficeSpec SHALL provide JSON output for direct workspace setup commands.

#### Scenario: Requesting JSON output
- **WHEN** a user passes `--json` to direct workspace setup commands
- **THEN** OfficeSpec SHALL print machine-readable output
...
- **THEN** OfficeSpec SHALL fail clearly
...
- **THEN** OfficeSpec SHALL avoid interactive prompts
