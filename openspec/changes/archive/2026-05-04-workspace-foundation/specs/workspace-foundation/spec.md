...
OfficeSpec SHALL give users and agents a recognizable workspace home for cross-repo planning.

#### Scenario: Planning across linked repos or folders
- **WHEN** a user creates an OfficeSpec workspace for repos or folders they plan across
...
- **GIVEN** a user runs OfficeSpec from a workspace root or one of its subdirectories
- **WHEN** OfficeSpec resolves the current workspace
...
- **GIVEN** a directory has `changes/` but is not an OfficeSpec workspace
- **WHEN** OfficeSpec resolves the current workspace
...
OfficeSpec SHALL use one folder-style workspace name across workspace identity, managed storage, and the local registry.

#### Scenario: Using one workspace name
- **WHEN** OfficeSpec creates or registers a managed workspace
...
- **WHEN** OfficeSpec accepts a workspace name
...
OfficeSpec SHALL distinguish a coordination workspace from a repo-local OfficeSpec project.

#### Scenario: Reading workspace identity
- **WHEN** OfficeSpec reads or writes workspace identity and workspace state
- **THEN** it SHALL use `.openspec-workspace/`

#### Scenario: Preserving repo-local OfficeSpec projects
- **GIVEN** a repo-local OfficeSpec project uses `openspec/`
- **WHEN** that repo is linked to a workspace
- **THEN** OfficeSpec SHALL continue treating `openspec/` as that repo's local OfficeSpec directory
...
- **WHEN** a user is working from an OfficeSpec workspace root
- **THEN** OfficeSpec SHALL treat that root as a workspace coordination surface
...
OfficeSpec SHALL keep shared workspace information separate from local machine paths.
...
- **WHEN** OfficeSpec stores local paths for a workspace
...
- **WHEN** OfficeSpec reads or writes local workspace paths
...
- **WHEN** OfficeSpec creates a workspace
...
OfficeSpec SHALL use a standard location for OfficeSpec-managed workspaces without asking most users to choose one.

#### Scenario: Using the standard workspace location
- **WHEN** OfficeSpec needs the location for OfficeSpec-managed workspaces
- **THEN** it SHALL use `<global-data-dir>/workspaces`
- **AND** `<global-data-dir>` SHALL follow existing OfficeSpec XDG and platform data directory behavior

#### Scenario: Avoiding workspace-specific storage overrides
- **WHEN** OfficeSpec resolves the location for OfficeSpec-managed workspaces
...
- **WHEN** OfficeSpec runs from native Windows shells such as PowerShell
- **AND** `XDG_DATA_HOME` is not set
- **THEN** OfficeSpec SHALL store managed workspaces under the Windows global data location
- **AND** paths SHALL follow native Windows path behavior

#### Scenario: Running from WSL2
- **WHEN** OfficeSpec runs from WSL2
- **THEN** OfficeSpec SHALL store managed workspaces under the Linux/XDG data location inside WSL
- **AND** paths SHALL follow Linux path behavior inside WSL

#### Scenario: Using the workspace location automatically
- **WHEN** OfficeSpec creates or resolves OfficeSpec-managed workspaces in later workflows
...
- **WHEN** OfficeSpec creates a workspace in the standard workspace location
...
- **WHEN** OfficeSpec resolves workspace paths or local repo paths
- **THEN** it SHALL interpret paths for the runtime running OfficeSpec
- **AND** Windows, UNC WSL, and WSL mount paths SHALL remain explicit user-provided paths

### Requirement: Local Workspace Registry
OfficeSpec SHALL keep a lightweight local registry of known workspaces on the current machine.

#### Scenario: Recording known workspaces
- **WHEN** OfficeSpec creates or learns about a managed workspace
...
- **WHEN** OfficeSpec reads workspace details
...
- **THEN** OfficeSpec MAY use the local registry to find known workspaces
- **AND** commands that need one workspace MAY use the registry to support an interactive picker

### Requirement: Stable Link Names
OfficeSpec SHALL use stable link names to refer to repos and folders in workspace planning.
...
OfficeSpec SHALL allow workspace planning to include linked repos and folders before they have repo-local OfficeSpec state.

#### Scenario: Planning with a repo that has not adopted OfficeSpec
...
- **THEN** OfficeSpec SHALL use the same planning model for both
...
- **WHEN** OfficeSpec records a link between a workspace and a local repo or folder
...
OfficeSpec SHALL treat workspace creation and detection as planning setup, not implementation.

#### Scenario: Creating or detecting a workspace
- **WHEN** a workspace exists
- **THEN** OfficeSpec SHALL treat it as a place for workspace-level planning
...
OfficeSpec SHALL keep repo ownership legible when planning happens in a workspace.
