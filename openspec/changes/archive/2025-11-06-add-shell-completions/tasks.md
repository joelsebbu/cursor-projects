...
- [x] Create `src/core/completions/command-registry.ts` with `COMMAND_REGISTRY` constant defining all OfficeSpec commands, flags, and metadata
...
- [ ] Test dynamic change/spec ID completion in real OfficeSpec projects
- [ ] Verify completion cache behavior (2-second TTL)
- [ ] Test behavior outside OfficeSpec projects (should skip dynamic completions)
