...
Users who run `openspec init` are left with files but no clear path to actually using the system. There's a gap between "I have OfficeSpec set up" and "I understand the workflow." An onboarding skill would guide users through their first complete change cycle on a real task in their codebase, teaching the workflow by doing it.

## What Changes

- Add new `/opsx:onboard` skill that guides users through their first OfficeSpec change
...
### New Capabilities
- `opsx-onboard-skill`: The onboarding skill that guides users through their first complete OfficeSpec workflow cycle with narration and codebase-aware task suggestions
