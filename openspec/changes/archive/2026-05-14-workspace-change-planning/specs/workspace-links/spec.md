...
OfficeSpec SHALL let users install OfficeSpec agent skills into a workspace during workspace setup.

#### Scenario: Prompting for workspace agent skills
- **WHEN** interactive workspace setup reaches agent skill installation
- **THEN** OfficeSpec SHALL ask which agents should get OfficeSpec skills in this workspace
- **AND** the prompt SHALL use agent-skill language rather than "AI tools" language

#### Scenario: Preselecting the preferred opener
- **GIVEN** the user selected a preferred opener that supports OfficeSpec skill generation
- **WHEN** interactive workspace setup asks which agents should get skills
- **THEN** OfficeSpec SHALL preselect the matching agent
...
- **THEN** OfficeSpec SHALL generate or refresh OfficeSpec skill files under the workspace root for each selected agent
...
- **THEN** OfficeSpec SHALL install workspace-local skills for the workflows selected by that profile
...
- **WHEN** workspace setup installs agent skills
- **THEN** OfficeSpec SHALL generate skill files only
...
- **WHEN** workspace setup installs agent skills
- **THEN** OfficeSpec SHALL still generate workspace-local skills only
...
- **WHEN** workspace setup installs agent skills
- **THEN** OfficeSpec SHALL leave linked repos and folders unchanged
...
- **THEN** OfficeSpec SHALL use the selected tool set for workspace agent skill installation
...
- **THEN** OfficeSpec SHALL create the workspace without installing agent skills
...
- **THEN** OfficeSpec SHALL include generated, refreshed, skipped, or failed skill installation results in machine-readable output

### Requirement: Workspace update manages agent skills
OfficeSpec SHALL provide a workspace update flow for refreshing agent skills after setup.
...
- **THEN** OfficeSpec SHALL update that current workspace
...
- **THEN** OfficeSpec SHALL update the `platform` workspace
...
- **THEN** OfficeSpec SHALL refresh OfficeSpec skills for selected agents
- **AND** it SHALL add skills for newly selected agents
- **AND** it SHALL remove OfficeSpec-managed workflow skill directories for agents that are no longer selected
...
- **THEN** OfficeSpec SHALL sync the workspace-local skill workflow set to the workflows selected by that profile
- **AND** deselected workflow skill directories SHALL be removed only when they are known OfficeSpec-managed workflow skill directories
...
- **GIVEN** global config delivery is `commands` or `both`
- **WHEN** workspace update refreshes workspace-local skills
- **THEN** OfficeSpec SHALL still update workspace-local skills only
...
- **THEN** OfficeSpec SHALL remove only known OfficeSpec-managed workflow skill directories
...
- **THEN** OfficeSpec SHALL replace the stored workspace-local selected agent list with that selection
...
- **THEN** OfficeSpec SHALL update workspace agent skills using that selected tool set
...
- **THEN** OfficeSpec SHALL refresh the stored selected agents using the active global profile
- **AND** it SHALL avoid prompting for agent selection

#### Scenario: Non-interactive update without stored selection
- **GIVEN** no workspace-local selected agents are stored
- **WHEN** non-interactive workspace update omits `--tools`
- **THEN** OfficeSpec SHALL complete without installing agent skills
...
- **WHEN** OfficeSpec reports workspace skill state
...
- **THEN** OfficeSpec SHALL report which agents were refreshed, added, removed, skipped, or failed

#### Scenario: Reporting workspace update results in JSON output
- **WHEN** workspace update runs with JSON output enabled
- **THEN** OfficeSpec SHALL include refreshed, added, removed, skipped, or failed skill results in machine-readable output

### Requirement: Workspace skill update surface is documented
OfficeSpec SHALL expose workspace skill setup/update behavior in user-facing command surfaces.

#### Scenario: Workspace update appears in help
- **WHEN** a user runs `openspec workspace --help`
- **THEN** OfficeSpec SHALL list `workspace update`
...
- **THEN** OfficeSpec SHALL document workspace selection options
