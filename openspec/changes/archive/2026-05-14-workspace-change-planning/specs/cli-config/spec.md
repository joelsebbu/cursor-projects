...
The `openspec config profile` command SHALL remain global while offering an explicit workspace apply path when run from inside an OfficeSpec workspace.
...
- **THEN** OfficeSpec SHALL save the global config changes
...
- **THEN** OfficeSpec SHALL run `openspec workspace update` for the current workspace
...
- **THEN** OfficeSpec SHALL explain that global config was updated
...
- **THEN** OfficeSpec SHALL not prompt to apply changes
...
- **THEN** OfficeSpec SHALL save the global config change without prompting to apply immediately
...
- **WHEN** the user runs `openspec config profile core`
- **THEN** OfficeSpec SHALL preserve existing repo-local shortcut behavior
...
- **GIVEN** the command runs in a path under a workspace planning home where a repo-local OfficeSpec project could also be detected
- **WHEN** OfficeSpec decides which apply prompt to show
...
- **AND** OfficeSpec SHALL not apply profile changes to a linked repo when the current planning home is the workspace

#### Scenario: Linked repo keeps repo-local profile behavior
- **GIVEN** a repo-local OfficeSpec project is registered as a workspace link
- **AND** the command runs from inside that linked repo rather than from the workspace planning home
- **WHEN** OfficeSpec decides which apply prompt or guidance to show
- **THEN** OfficeSpec SHALL preserve repo-local `openspec update` behavior for that repo
