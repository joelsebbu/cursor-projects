...
OfficeSpec SHALL support workspace-level changes whose shared plan lives in the workspace planning home.

#### Scenario: Creating a workspace change
- **GIVEN** the command runs from an OfficeSpec workspace
- **WHEN** the user creates a change for workspace planning
- **THEN** OfficeSpec SHALL create the change under the workspace planning path
...
- **WHEN** OfficeSpec reports or creates planning artifacts for that change
...
- **THEN** OfficeSpec SHALL capture the product goal at the workspace change level
...
- **WHEN** OfficeSpec creates a workspace-level change
- **THEN** it SHALL not create repo-local OfficeSpec change directories inside linked repos or folders
- **AND** it SHALL not edit implementation files in linked repos or folders

### Requirement: Workspace affected areas
OfficeSpec SHALL represent ownership or implementation boundaries in a workspace change as affected areas.
...
- **THEN** OfficeSpec SHALL validate those area names against the workspace links
...
- **THEN** OfficeSpec SHALL allow the shared plan to exist before all affected areas are finalized
...
- **WHEN** OfficeSpec reports or creates workspace-scoped specs
...
- **THEN** OfficeSpec SHALL distinguish affected areas from delivery slices or phases
...
OfficeSpec SHALL keep the workspace change plan as the source of truth until implementation begins for a selected affected area.
...
- **THEN** OfficeSpec SHALL require an explicit implementation workflow with a selected affected area
