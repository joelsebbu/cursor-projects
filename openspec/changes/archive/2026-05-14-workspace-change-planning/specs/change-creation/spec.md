...
- **WHEN** the user creates a new change
- **THEN** OfficeSpec SHALL resolve the current workspace as the planning home
...
- **THEN** OfficeSpec SHALL preserve repo-local change creation behavior for that location
...
- **GIVEN** the command runs outside an OfficeSpec workspace
- **WHEN** the user creates a new change in a repo-local OfficeSpec project
- **THEN** OfficeSpec SHALL continue to create the change under `openspec/changes/`
...
- **THEN** OfficeSpec SHALL reject those invalid affected areas
...
- **THEN** OfficeSpec SHALL create the workspace change
