...
OfficeSpec SHALL store a workspace's preferred opener in machine-local workspace state when the user explicitly chooses one.
...
- **THEN** OfficeSpec SHALL record the opener in `.openspec-workspace/local.yaml`
...
- **THEN** OfficeSpec SHALL record `preferred_opener.kind` as `agent`
...
- **THEN** OfficeSpec SHALL leave the workspace preferred opener unset
- **AND** the unset state SHALL allow `workspace open` to prompt later

#### Scenario: Supported preferred opener values
- **WHEN** OfficeSpec accepts a preferred opener value
...
- **THEN** OfficeSpec SHALL show all supported openers
...
OfficeSpec SHALL maintain files that make a workspace directly openable after setup and link changes.

#### Scenario: Creating the open surface during setup
- **WHEN** `openspec workspace setup` creates a workspace
- **THEN** OfficeSpec SHALL create or refresh `AGENTS.md`
...
- **THEN** OfficeSpec SHALL refresh `AGENTS.md`
...
- **WHEN** OfficeSpec refreshes `<workspace-name>.code-workspace`
...
- **WHEN** OfficeSpec refreshes workspace ignore rules
...
- **GIVEN** `AGENTS.md` contains content outside the OfficeSpec workspace guidance markers
- **WHEN** OfficeSpec refreshes workspace guidance
- **THEN** it SHALL replace only the marked OfficeSpec workspace guidance block
- **AND** it SHALL preserve content outside the markers

#### Scenario: Appending AGENTS guidance when markers are missing
- **GIVEN** `AGENTS.md` exists and OfficeSpec workspace guidance markers are absent
- **WHEN** OfficeSpec refreshes workspace guidance
- **THEN** it SHALL append the marked OfficeSpec workspace guidance block
