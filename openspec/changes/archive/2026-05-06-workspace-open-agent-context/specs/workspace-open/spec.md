...
OfficeSpec SHALL provide a `workspace open` command that opens an OfficeSpec workspace working set through an agent or VS Code editor.

#### Scenario: Opening the current workspace
- **GIVEN** the command runs from inside an OfficeSpec workspace
- **WHEN** the user runs `openspec workspace open`
- **THEN** OfficeSpec SHALL open that current workspace
...
- **THEN** OfficeSpec SHALL open the `platform` workspace
...
- **THEN** OfficeSpec SHALL fail with a clear conflict error
...
- **THEN** OfficeSpec SHALL fail with a clear error that the root workspace open surface supports launching through a selected opener
...
- **THEN** OfficeSpec SHALL fail with a clear error that this slice supports root workspace open
...
OfficeSpec SHALL resolve the workspace to open using current workspace context, local registry state, and interactive selection.
...
- **WHEN** the user runs `openspec workspace open`
- **THEN** OfficeSpec SHALL open the current workspace
...
- **WHEN** the user runs `openspec workspace open`
- **THEN** OfficeSpec SHALL open that known workspace directly
...
- **WHEN** the user runs `openspec workspace open`
- **THEN** OfficeSpec SHALL present a picker with workspace names and locations
...
- **WHEN** the user runs `openspec workspace open`
- **THEN** OfficeSpec SHALL fail with a clear message listing the known workspace names
...
OfficeSpec SHALL resolve the opener from command overrides, workspace-local preference, or an interactive prompt.

#### Scenario: Conflicting opener overrides
- **WHEN** the user runs `openspec workspace open --agent codex --editor`
- **THEN** OfficeSpec SHALL fail with a clear conflict error naming `--agent` and `--editor`
...
- **THEN** OfficeSpec SHALL use the stored preferred opener
...
- **THEN** OfficeSpec SHALL use Codex for that open command
- **AND** it SHALL leave the stored preferred opener unchanged

#### Scenario: Overriding with VS Code editor for one session
- **GIVEN** the workspace has a stored preferred opener
- **WHEN** the user runs `openspec workspace open --editor`
- **THEN** OfficeSpec SHALL open the workspace in VS Code editor mode
- **AND** it SHALL leave the stored preferred opener unchanged
...
- **AND** the terminal is interactive
- **WHEN** the user runs `openspec workspace open` using default opener resolution
- **THEN** OfficeSpec SHALL prompt the user to choose an opener
...
- **WHEN** the user runs `openspec workspace open` using default opener resolution
- **THEN** OfficeSpec SHALL fail with a clear message that no supported opener is available
...
OfficeSpec SHALL launch the selected opener using existing workspace files and linked path state.

#### Scenario: Opening VS Code editor
- **GIVEN** the user selected the VS Code editor opener
- **WHEN** `code` is available on `PATH`
- **THEN** OfficeSpec SHALL open the workspace's maintained `.code-workspace` file with VS Code
...
- **THEN** OfficeSpec SHALL launch Codex from the workspace root
...
- **THEN** OfficeSpec SHALL launch Claude from the workspace root
...
- **WHEN** the user runs `openspec workspace open`
- **THEN** OfficeSpec SHALL fail with a clear error naming the missing executable
...
- **WHEN** the user runs `openspec workspace open`
- **THEN** OfficeSpec SHALL fail with a clear error naming `code`
...
OfficeSpec SHALL make linked repos and folders visible for workspace exploration and planning before change creation.
...
- **THEN** OfficeSpec SHALL include every valid linked path in the opened working set
...
- **THEN** OfficeSpec SHALL skip the broken linked path
...
#### Scenario: Opening links with repo-local OfficeSpec state absent
- **GIVEN** a linked repo or folder has a valid local path and repo-local `openspec/` state is absent
- **WHEN** the user opens the workspace
- **THEN** OfficeSpec SHALL include that link when its local path is valid
- **AND** it SHALL treat missing repo-local OfficeSpec state as an implementation-readiness concern for later workflows while continuing open

### Requirement: Workspace Open Guidance
OfficeSpec SHALL use durable workspace guidance as the primary context source for root workspace open.

#### Scenario: Launching with existing workspace guidance
- **GIVEN** the workspace has OfficeSpec-managed guidance in `AGENTS.md`
- **WHEN** the user opens the workspace
- **THEN** OfficeSpec SHALL refresh the maintained `.code-workspace` from current linked path state
...
- **WHEN** OfficeSpec launches that opener
- **THEN** OfficeSpec SHALL use a minimal prompt such as `Open this OfficeSpec workspace.`
