...
- **AND** ensure each file begins with YAML frontmatter that contains only a `description: <stage summary>` field followed by the shared OfficeSpec workflow instructions wrapped in managed markers
...
- **AND** each template includes instructions for the relevant OfficeSpec workflow stage
...
- **AND** wrap the generated content in OfficeSpec managed markers so `openspec update` can safely refresh the commands
...
- **AND** populate each file from shared templates (wrapped in OfficeSpec markers) so workflow text matches other tools
- **AND** each template includes instructions for the relevant OfficeSpec workflow stage
...
- **AND** wrap the generated content in OfficeSpec markers so `openspec update` can refresh the prompts without touching surrounding custom notes
...
- **AND** wrap the shared template body with OfficeSpec markers so `openspec update` can refresh the content
- **AND** each template includes instructions for the relevant OfficeSpec workflow stage
...
- **AND** populate each file as TOML that sets a stage-specific `description = "<summary>"` and a multi-line `prompt = """` block with the shared OfficeSpec template
- **AND** wrap the OfficeSpec managed markers (`<!-- OPENSPEC:START -->` / `<!-- OPENSPEC:END -->`) inside the `prompt` value so `openspec update` can safely refresh the body between markers without touching the TOML framing
...
- **AND** populate each file from shared templates so command text matches other tools
- **AND** include simple Markdown headings (e.g., `# OfficeSpec: Proposal`) without YAML frontmatter
- **AND** wrap the generated content in OfficeSpec managed markers where applicable so `openspec update` can safely refresh the commands
- **AND** each template includes instructions for the relevant OfficeSpec workflow stage
