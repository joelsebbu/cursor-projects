...
The `global-config` spec defines how OfficeSpec reads/writes `config.json`, but users currently must edit it by hand. This command provides a CLI interface to that config.
