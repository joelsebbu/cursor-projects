...
humans and agents should experience a context store as a normal OfficeSpec root
...
That means durable planning state lives in normal OfficeSpec artifacts:
...
- "Create a context store I can use as a normal OfficeSpec place for specs and
  changes."
...
- Whether the folder is a healthy OfficeSpec root.
...
- Whether a refusal means "unsafe folder", "not an OfficeSpec root", "missing
...
- Setup creates or preserves a standalone OfficeSpec root, then adds thin
...
- A healthy OfficeSpec root contains `openspec/`, a config file
...
- Setup accepts missing directories, empty directories, Git-only directories,
  and existing healthy OfficeSpec roots.
...
- Registering a healthy OfficeSpec root without context-store identity should ask
...
A human or agent asks OfficeSpec to create a new context store in a missing or
empty directory.
...
- `.git/` is preserved.
- The normal OfficeSpec root and context-store identity are created.
- The command does not stage, commit, push, create remotes, or define Git
  workflow policy.

### Existing Healthy Root Setup

A human already has a standalone OfficeSpec root and wants it to become a context
store.
...
- `context-store register <path>` validates the clone as a healthy OfficeSpec
...
A human has a normal OfficeSpec root that does not yet have
...
- Existing healthy OfficeSpec root.
...
Next: use normal OfficeSpec specs and changes in this store.
```
...
- An existing healthy OfficeSpec root with valid `.openspec-store/store.yaml`.
- An existing healthy OfficeSpec root without identity only after clear
  confirmation.

Reject:

- Missing paths.
- Partial OfficeSpec roots.
...
```text
Turn this OfficeSpec root into context store "team-context"?
```
...
- Create missing OfficeSpec directories.
...
### 1. Add An OfficeSpec Root Helper

Create `src/core/openspec-root.ts`.

Responsibilities:

- Define canonical relative paths for a normal OfficeSpec root.
...
- Missing path: create root and full OfficeSpec shape.
- Empty path: create full OfficeSpec shape.
- Git-only path: preserve `.git/`, create full OfficeSpec shape.
- Healthy OfficeSpec root: preserve root content, add identity if missing.
...
1. Ensure the OfficeSpec root shape if setup is allowed.
...
Update setup JSON so `created_files` includes both OfficeSpec-root paths and
...
- The path must exist and be a healthy OfficeSpec root.
- Existing valid `.openspec-store/store.yaml` supplies or confirms the store id.
- A healthy OfficeSpec root without identity can be converted only after user
  confirmation.
...
OfficeSpec-root inspection results.

Update `src/commands/context-store.ts` output types and printers so:

- Human doctor output names OfficeSpec-root health separately.
...
so the next step points toward normal OfficeSpec specs and changes.
...
- "Use normal OfficeSpec specs and changes in this store."
- "This store is a standalone OfficeSpec root."
...
- Setup preserves an existing healthy OfficeSpec root and config edits.
- Setup creates config in JSON/non-interactive mode without tool selection.
- Setup rejects arbitrary non-empty folders and creates no OfficeSpec files.
...
- A fresh setup leaves a normal OfficeSpec root plus
  `.openspec-store/store.yaml`.
...
- Register can turn a healthy OfficeSpec root into a context store only after
  confirmation.
