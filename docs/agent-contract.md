...
- **Store banner.** In human mode, a store-selected root prints `Using OfficeSpec root: <id> (<path>)` to stderr. Never printed in JSON mode.
...
All root-resolving commands (`list`, `show`, `validate`, `status`, `instructions`, `instructions apply`, `instructions archive`, `new change`, `archive`, `doctor`, `context`, `schemas`) resolve one OfficeSpec root with one precedence:
...
### OfficeSpec-root health (error, no fix)
