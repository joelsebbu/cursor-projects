...
git switch -c add-dark-mode        start a branch, as usual
   │
/ofsx:propose improve-expense-approvals        draft the plan (proposal + specs + tasks)
   │
REVIEW THE PLAN                    you read it before any code — see Reviewing a Change
   │
/ofsx:apply                        build it; artifacts + code change together
   │
git commit && open a PR            the PR contains the spec delta AND the code
   │
teammate reviews, merges
   │
/ofsx:archive                      fold the delta into specs/, move the change to archive/
```
...
Pick one and be consistent. Either way, `/ofsx:archive` checks that tasks are complete and offers to sync first, so nothing merges half-finished by accident.
...
- **The one place conflicts show up is `specs/`.** If two changes both modify the *same* requirement, archiving the second one will conflict in `openspec/specs/…/spec.md` — resolve it like any merge conflict, keeping the requirement that reflects reality. This is rare, and it's a feature: it's git telling you two changes disagreed about how the process should behave.
