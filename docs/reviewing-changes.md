...
```
/ofsx:propose ──► REVIEW THE PLAN ──► /ofsx:apply ──► REVIEW THE CODE ──► /ofsx:archive
                  (before any code)                    (/ofsx:verify)
```

1. **After `/ofsx:propose`** (or `/ofsx:ff`), before `/ofsx:apply` — read the plan while it's still just words.
2. **After building**, with `/ofsx:verify` — check that the code actually did what the plan said.
...
├── design.md        (only for bigger changes) — the approach and plan
...
The process SHALL let a user switch between light and dark themes.
...
- **A vague requirement.** "The process SHALL be fast" can't be built or tested. What's fast?
...
Read the deltas asking *would I be happy if the process did exactly — and only — this?* Nothing here is about code yet, so it stays cheap to change.

## The tasks: is the plan of work sane?

Open `tasks.md` last. It's the action checklist the AI will work through.
...
Once the work is built, `/ofsx:verify` is your second review. It re-reads the artifacts and the code and reports mismatches across three dimensions:
...
It flags issues as CRITICAL, WARNING, or SUGGESTION, and it does **not** block archiving — it surfaces the gaps and leaves the call to you. This is the difference between "did the AI do the work" and "did it build what we agreed."

`/ofsx:verify` is in the expanded profile. If you don't have it, turn it on with `openspec config profile` (then `openspec update`), or just re-read the change and the diff yourself.
...
If all seven pass, run `/ofsx:apply` with confidence. If any fail, that's not a setback — it's the two minutes doing its job.
