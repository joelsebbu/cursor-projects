...
Rather not do this by hand? Paste the prompt below into any AI assistant that can run shell commands — Claude Code, Codex, Cursor, Gemini CLI, Copilot, and the rest of the [supported tools](supported-tools.md). It installs the CLI, initializes this project, and reports back what actually happened.
...
```text
Install OfficeSpec in this project and set it up for me. Follow these steps in
order, and stop where a step tells you to stop.

1. RUNTIME. Run `node --version`. OfficeSpec needs Node.js 20.19.0 or higher. If
...
2. INSTALL. Use whichever package manager is already on my PATH, preferring npm:
     ...
     bun add -g officespec@latest
     yarn global add officespec@latest   (Yarn 1.x only)
   Don't pick based on this folder's lockfile — a global install has nothing to
   ...
   point me at the Nix section of the OfficeSpec installation docs instead.)
   Show me the exact command and let me confirm before you run it; this installs
   work outside the project, and I may want a different package manager to
   own it.
...
```bash
yarn global add officespec@latest
```

Yarn 2 and later (Berry) removed the `global` command. On those versions, install OfficeSpec with npm, pnpm, or bun instead — a global CLI doesn't need to share your work's package manager.
...
deno install --global \
  ...
  npm:officespec@latest
# or
deno install --global \
  --allow-read --allow-write --allow-env --allow-sys=cpus,homedir --allow-net=edge.openspec.dev \
  npm:officespec@^1.3.1
```
...
Bun can install OfficeSpec globally, but OfficeSpec currently runs on Node.js.
...
Run OfficeSpec directly without installation:

```bash
nix run github:joelsebbu/OpenSpec -- init
```

Or install to your profile:

```bash
nix profile install github:joelsebbu/OpenSpec
```
...
```nix
{
  inputs = {
    ...
    openspec.url = "github:joelsebbu/OpenSpec";
  };
...
```bash
npm install -g officespec@latest   # or pnpm/yarn/bun equivalent
...
There's no `openspec uninstall` command, because OfficeSpec is just a global package plus some files in your work. Removing it is a few manual steps, and nothing here touches your source code.

**1. Remove the global package:**

```bash
npm uninstall -g officespec   # or: pnpm rm -g / yarn global remove / bun rm -g
```

**2. Remove OfficeSpec from a project (optional).** Delete the `openspec/` directory if you no longer want its specs and changes:
...
After installing, initialize OfficeSpec in your work:
