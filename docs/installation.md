...
Yarn 2 and later (Berry) removed the `global` command. On those versions, install OpenSpec with npm, pnpm, or bun instead — a global CLI doesn't need to share your work's package manager.
...
There's no `openspec uninstall` command, because OpenSpec is just a global package plus some files in your work. Removing it is a few manual steps, and nothing here touches your source code.
...
After installing, initialize OpenSpec in your work:
