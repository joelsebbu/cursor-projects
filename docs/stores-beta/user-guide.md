...
- Your work spans several repos — one feature touches the operations, the
  team site, and a shared docs. Whose `openspec/` folder does the plan
  live in?
...
- Requirements are owned by one team and consumed by others. The wiki
  version drifts, and your AI assistant can't read it anyway.
...
**Connecting the team's work folders.** A code repo whose planning is fully
...
work folders that all plan into the same store, set it once, globally,
...
repo still needs its own work tasks, branch, and review.
...
A workset is a convenient way to open the planning store and both work folders
together:
...
together every session — the planning repo plus two or three work folders.
