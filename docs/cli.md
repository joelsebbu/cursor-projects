...
|----------|----------|---------|
| **Setup** | `init`, `update` | Initialize and update OfficeSpec in your work |
| **Stores (standalone OfficeSpec repos)** | `store setup`, `store register`, `store unregister`, `store remove`, `store list`, `store doctor` | Manage stores — standalone OfficeSpec repos you've registered |
...
| `openspec new change <id>` | Create repo-local change scaffolding | `--json`, plus `--store <id>` to use a registered store as the OfficeSpec root |
...
Initialize OfficeSpec in your work. Creates the folder structure and configures AI tool integrations.
...
field so OfficeSpec never overwrites project-specific guidance.
...
Update OfficeSpec instruction files after upgrading the CLI. Re-generates AI tool configuration files using your current global profile, selected workflows, and delivery mode.
...
```text
A newer OfficeSpec CLI is available (v1.6.0 → v1.7.0).
  Running from: /usr/local/lib/node_modules/officespec
? Upgrade to v1.7.0 now? (Y/n)
```

Answer yes and it runs `npm install -g officespec@latest`, then re-runs the update with the new CLI so the new workflows land in the same command. It confirms the upgrade by asking the installed binary its version rather than trusting npm's exit code, so if another install earlier on your `PATH` is still answering, it tells you instead of claiming success. Answer no and it prints the command and updates with the CLI you have. Ctrl-C stops the command.
...
| How OfficeSpec is installed | What you get |
|---------------------------|--------------|
...
| An `npx` / `dlx` cache | `npx officespec@latest update` — that command is the update, so there is no second step |
...
them, so OfficeSpec compares that against the installed CLI. Command files carry no
version stamp, so for a tool that has commands but no skills (delivery
`commands`), OfficeSpec compares the file contents against what it would generate
...
way, generated files are OfficeSpec's to own — keep your own instructions
elsewhere.

---

## Stores (standalone OfficeSpec repos)
...
A store is a standalone OfficeSpec repo you've registered on this machine — for example a planning repo or a contracts repo. Registering a store lets normal commands (`list`, `show`, `status`, `validate`, `new change`, `archive`, ...) act in it from anywhere by passing `--store <id>`.

### `openspec store setup`

Create and register a local store. With no arguments in a terminal,
OfficeSpec guides the user through setup. Agents and scripts should pass explicit
inputs and use `--json`.
...
Non-interactive runs (`--json`, scripts, agents) must pass both the store id and `--path`. In an interactive terminal, setup prompts for the location with an editable suggestion in a visible, user-owned place (for example `~/openspec/<id>`); it never defaults to OfficeSpec's managed data directory.
...
| `--yes` | Confirm creating store identity metadata for a healthy OfficeSpec root |
| `--json` | Output JSON |
...
shown by OfficeSpec on this machine.
...
OfficeSpec refuses to delete a folder that does not contain matching
store metadata.
...
Recording a remote is not sync: OfficeSpec never clones, pulls, or pushes on its own.
...
One read-only question, one place: is the OfficeSpec root healthy, and are the stores it references available on this machine?
...
Everything this work relates to through OfficeSpec declarations, in one working set: the OfficeSpec root and the stores it references.
...
List changes or specs in your work.
...
Opens a terminal-based interface for navigating your work's specifications and changes.
...
When the change removes the capability's last requirement, OfficeSpec deletes its
...
Create a change directory and optional checked-in metadata in the resolved OfficeSpec root.
...
| `--store <id>` | Store id to use as the OfficeSpec root (a store is a standalone OfficeSpec repo you've registered) |
| `--json` | Output JSON |
...
whether work tasks are complete. `isComplete` is retained as a
...
|--------|-------------|
| `--json` | Output as JSON |
| `--store <id>` | Use a registered store as the OfficeSpec root |
...
Available schemas:
  ...
  spec-driven (package)
    The default spec-driven work workflow
...
Copy an existing schema to your work for customization.
...
spec-driven resolves from: package
  Source: /usr/local/lib/node_modules/officespec/schemas/spec-driven
```
...
View and modify global OfficeSpec configuration.
...
If there are no config changes but the current project files are out of sync with your global profile/delivery, OfficeSpec will show a warning and suggest `openspec update`.
...
Submit feedback about OfficeSpec. Creates a GitHub issue.
...
Manage shell completions for the OfficeSpec CLI.
...
`$env:PROFILE` tells OfficeSpec which profile to configure in this session. The
