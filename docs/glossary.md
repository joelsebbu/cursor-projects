...
**Spec.** A document describing how part of your system behaves. Specs live in `openspec/specs/`, are organized by domain, and are made of requirements and scenarios. The spec is the agreed-upon answer to "what does this work do?" See [Concepts](concepts.md#specs).
...
**Requirement.** A single behavior the process must have, usually written with an RFC 2119 keyword: "The process SHALL expire sessions after 30 minutes." Requirements state the *what*, not the *how*.
...
**Design (`design.md`).** The *how*: approach and plan, architecture decisions, and the files you expect to touch. Optional for simple changes.

**Tasks (`tasks.md`).** The action checklist, with checkboxes. The AI works through it during `/ofsx:apply` and checks items off as it goes.
...
**Sync.** Merging a change's delta specs into the main specs *without* archiving the change. Usually automatic (archive offers to do it), but available on its own as `/ofsx:sync` for long-running changes. See [Commands](commands.md#ofsxsync).

## Workflow and commands

**OFSX.** The current standard OfficeSpec workflow, built around fluid actions instead of rigid phases. Its slash commands all start with `/ofsx:`. See [OFSX Workflow](ofsx.md).

**Slash command.** A command you type into your AI assistant's chat, like `/ofsx:propose`. Slash commands drive the workflow. They are not terminal commands. See [How Commands Work](how-commands-work.md).

**Explore (`/ofsx:explore`).** The thinking-partner command. It reads your workspace, compares options, and clarifies a fuzzy idea into a concrete plan, creating no artifacts and writing no code. The recommended starting point whenever you have a problem but not yet a plan. See [Explore First](explore.md).
...
**Command file.** A per-tool slash command file (`.../commands/ofsx-*`). The older delivery mechanism, still supported alongside skills. You rarely touch these directly.
