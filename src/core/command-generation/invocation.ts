 ...
 *
 * - The *name* comes from the file. `.../commands/ofsx/<id>.md` is namespaced
 *   by its directory, so the tool registers `ofsx:<id>` (Claude Code, Gemini,
 *   Crush, ...). `.../commands/ofsx-<id>.md` names the command with the
 *   filename, so the tool registers `ofsx-<id>` (Cursor, GitHub Copilot,
 *   OpenCode, ...).
 ...
 *   invoked with `@` (`@ofsx-propose`), so its adapter declares that prefix.
 *
 ...
 *
 * The test is the filename, not the directory: an `ofsx-` prefix means the
 * filename is the command. Every other shape is treated as namespaced, which
 * is what all seven `ofsx/<id>.*` adapters need. An adapter that neither
 * prefixes the filename nor nests under `ofsx/` would land here too — none
 ...
 * @returns 'flat' when the filename carries the `ofsx-` prefix, otherwise
 *          'namespaced'
 */
export function getInvocationStyleForPath(commandFilePath: string): CommandInvocationStyle {
  return path.basename(commandFilePath).startsWith('ofsx-') ? 'flat' : 'namespaced';
}
 ...
 * @returns What the user types, e.g. `/ofsx:apply`, `/ofsx-apply`, `@ofsx-apply`
 */
export function formatCommandInvocation(
  ...
  return `${invocation.prefix}ofsx${separator}${commandId}`;
}

/**
 * Whether a tool's invocation differs from the canonical `/ofsx:<id>` that
