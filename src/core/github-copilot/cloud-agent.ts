 ...
 * These files enable the GitHub Copilot AI assistant (cloud) to use the
 * OfficeSpec CLI in its ephemeral dev environment.
 */
...
const OPENSPEC_MANAGED_MARKER = 'Generated by OfficeSpec for GitHub Copilot coding agent support.';
 ...
 * This workflow pre-installs the OfficeSpec CLI in the Copilot AI assistant's
...
jobs:
  # The job MUST be called \`copilot-setup-steps\` for Copilot AI assistant to pick it up.
  copilot-setup-steps:
    ...
    steps:
      ...
      - name: Install OfficeSpec CLI
        run: npm install -g officespec

      - name: Verify OfficeSpec CLI
 ...
 * This tells the GitHub Copilot AI assistant how to use the OfficeSpec CLI.
 */
...
function generateCopilotAgentFileBody(includeManagedMarker = false): string {
  ...
  return `---
name: OfficeSpec
description: "Manages OfficeSpec changes, specs, and workflows using the OfficeSpec CLI. Use this agent for proposing changes, exploring ideas, validating artifacts, checking status, and archiving completed work."
tools:
  - "execute"
  - "read"
  - "search"
...
${managedMarker}# OfficeSpec Agent

You are a specialized agent for managing OfficeSpec workflows. Before using the \`openspec\` CLI, run \`openspec --version\`. If it is unavailable, install it with \`npm install -g officespec\`.

## What is OfficeSpec?

OfficeSpec is a structured change management system for teams. It organizes work into **changes** with planning artifacts (proposals, specs, plans, tasks) that guide delivery.
...
| Command | Purpose |
|---------|---------|
| \`openspec init\` | Initialize OfficeSpec in the project |
| \`openspec update\` | Update OfficeSpec configuration and artifacts |
| \`openspec view\` | Interactive dashboard |
| \`openspec config\` | View or modify settings |

## Workflow

When asked to work with OfficeSpec, follow this pattern:
...
- \`openspec/\` — Root OfficeSpec directory
...
function generatePreviousCopilotAgentFileBody(includeManagedMarker = false): string {
  ...
  content = replaceRequired(
    content,
    'You are a specialized agent for managing OfficeSpec workflows. Before using the `openspec` CLI, run `openspec --version`. If it is unavailable, install it with `npm install -g officespec`.',
    'You are a specialized agent for managing OfficeSpec workflows. You have access to the `openspec` CLI through shell commands, pre-installed in the development environment via `copilot-setup-steps.yml`.',
...
3. Run \`openspec validate --json\` to verify the artifacts are well-formed`,
  ...
  content = replaceRequired(
    content,
    'You are a specialized agent for managing OfficeSpec workflows. You have access to the `openspec` CLI through shell commands, pre-installed in the development environment via `copilot-setup-steps.yml`.',
    'You are a specialized agent for managing OfficeSpec workflows. You have access to the `openspec` CLI which is pre-installed in the development environment via `copilot-setup-steps.yml`.',
 ...
 * currently hold user-owned, non-managed content — i.e. files OfficeSpec will
...
/**
 * Return the managed cloud-file paths (relative to the project root) that
 * currently exist and hold OfficeSpec-generated content. Callers report this
