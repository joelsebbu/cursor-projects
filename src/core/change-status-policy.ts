...
export function buildActionContext(input: ActionContextInput): ActionContext {
  return {
    ...
    constraints: ['Folder-local change artifacts and work edits are scoped to this workspace.'],
  };
}
...
export function buildNextSteps(input: ChangeNextStepsInput): string[] {
  ...
  if (readyArtifact) {
    ...
    steps.push(
      `All planning artifacts are complete. Run openspec instructions apply --change "${input.changeName}"${storeFlag} --json to inspect work progress.`
    );
  }
