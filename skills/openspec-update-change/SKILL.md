...
**Guardrails**
- Planning artifacts only - NEVER do the work early. If the revised plan implies work changes, stop and point to `/openspec-apply-change`.
- Use the artifact ids and paths reported by `openspec status`; never branch on hardcoded artifact names.
- Edit only the concrete files in `existingOutputPaths`; never write to a glob `resolvedOutputPath`.
- Do not advance the work frontier: no new artifacts, no new files under glob artifacts - that is `/openspec-continue-change`'s job.
