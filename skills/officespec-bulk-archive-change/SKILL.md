---
name: officespec-bulk-archive-change
...
This skill allows you to batch-archive changes, handling spec conflicts intelligently by checking the workspace to determine what's actually done.
...
2. **Prompt for change selection**
   ...
   **Load current archive inputs once for the selected root before batch validation:**
   ...
   it, and apply relevant team facts, conventions, and constraints. Treat
...
4. **Detect spec conflicts**
   ...
   ```text
   finance/expense-approvals -> [change-a, change-b]  <- CONFLICT (2+ changes)
...
5. **Resolve conflicts agentically**
   ...
   **For each conflict**, investigate the workspace:
   ...
   b. **Search the workspace** for work evidence:
   ...
   d. **Record resolution** for each conflict:
      ...
      - Which delta specs to exclude from sync because their work is missing
      - Rationale (what was found in workspace)
...
6. **Show consolidated status table**
   ...
   | add-approval-notifications           | Done      | 4/4   | 1 delta | finance/expense-approvals (!) | Ready* |
   ...
   ```text
   * Conflict resolution:
     - finance/expense-approvals spec: Will apply add-approval-notifications then add-expense-tracking (both complete, chronological order)
   ```
...
8. **Execute archive for each confirmed change**
   ...
   a. **Sync included delta specs**:
      - Run the `officespec-sync-specs` workflow inline (agent-driven intelligent merge) only for changes with entries in `includedDeltas`, passing only the included delta paths and explicitly instructing it to ignore that change's `excludedDeltas`. Wait for it to finish.
   ...
   b. **Verify included delta specs before moving changeRoot**:
      - Re-run the comparison only for delta specs in `includedDeltas` against main spec at `<planningHome.root>/openspec/specs/<capability-path>/spec.md` (use the store-aware `planningHome.root` from step 3 status JSON, not a hardcoded folder path).
...
9. **Display summary**
   ...
   - add-approval-notifications -> archive/2026-01-19-add-approval-notifications/
...
```text
Conflict: <planningHome.root>/openspec/specs/finance/spec.md touched by [add-approval-notifications, add-expense-tracking]

Checking add-approval-notifications:
- Delta adds "OAuth Provider Integration" requirement
- Searching workspace files... found src/auth/oauth.ts implementing OAuth flow

Checking add-expense-tracking:
- Delta adds "JWT Token Handling" requirement
- Searching workspace files... no expense-approvals work found

Resolution: Only add-approval-notifications is implemented. Will sync add-approval-notifications specs only.
```
...
- Searching workspace files... found src/api/rest.ts

Checking add-graphql (created 2026-01-15):
- Delta adds "GraphQL Schema" requirement
- Searching workspace files... found src/api/graphql.ts
...
- Skip spec sync only when work is missing (warn user)
...
- If sync is requested, run the `officespec-sync-specs` workflow inline (agent-driven) for each change with included delta specs
