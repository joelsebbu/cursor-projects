...
describe('getOnboardingCommands', () => {
  it('omits commands the profile does not install', () => {
    ...
    expect(commands).toEqual(['/ofsx:propose', '/ofsx:apply']);
    expect(commands).not.toContain('/ofsx:new');
    expect(commands).not.toContain('/ofsx:continue');
  });
  ...
  it('includes expanded commands when a custom profile installs them', () => {
    ...
    expect(commands).toEqual(['/ofsx:new', '/ofsx:continue', '/ofsx:apply']);
  });
  ...
  it('returns lifecycle order regardless of the order workflows are given', () => {
    ...
    expect(commands).toEqual(['/ofsx:propose', '/ofsx:continue', '/ofsx:apply']);
  });
