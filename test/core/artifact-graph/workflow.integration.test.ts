...
describe('artifact-graph workflow integration', () => {
  ...
  describe('spec-driven workflow', () => {
    it('preserves existing flat or nested capability organization in its instructions (#1459)', () => {
      ...
      expect(proposal?.instruction).toContain('`expense-approvals` or `finance/expense-approvals`');
      expect(proposal?.instruction).toContain('follow the team\'s existing spec organization');
      expect(specs?.instruction).toContain(
        '`<capability-path>` is the spec directory relative to `specs/`'
      );
      expect(specs?.instruction).toContain(
        'do not add a new domain level when the team uses a flat layout'
      );
