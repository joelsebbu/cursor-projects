...
describe('tool-detection', () => {
  ...
  describe('SKILL_NAMES', () => {
    it('should contain all skill names matching COMMAND_IDS', () => {
      ...
      expect(SKILL_NAMES).toContain('officespec-explore');
      expect(SKILL_NAMES).toContain('officespec-new-change');
      expect(SKILL_NAMES).toContain('officespec-continue-change');
      expect(SKILL_NAMES).toContain('officespec-apply-change');
      expect(SKILL_NAMES).toContain('officespec-update-change');
      expect(SKILL_NAMES).toContain('officespec-ff-change');
      expect(SKILL_NAMES).toContain('officespec-sync-specs');
      expect(SKILL_NAMES).toContain('officespec-archive-change');
      expect(SKILL_NAMES).toContain('officespec-bulk-archive-change');
      expect(SKILL_NAMES).toContain('officespec-verify-change');
      expect(SKILL_NAMES).toContain('officespec-onboard');
      expect(SKILL_NAMES).toContain('officespec-propose');
    });
  });
  ...
  describe('getToolSkillStatus', () => {
    ...
    it('should detect when one skill exists', async () => {
      const skillDir = path.join(testDir, '.claude', 'skills', 'officespec-explore');
    ...
    it('should detect legacy Codex skills before they are migrated', async () => {
      const skillDir = path.join(testDir, '.codex', 'skills', 'officespec-explore');
      await fs.mkdir(skillDir, { recursive: true });
  ...
  describe('getToolStates', () => {
    ...
    it('should expose only the marked owner of a shared skill tree as configured', async () => {
      ...
      const skillDir = path.join(skillsDir, 'officespec-explore');
      await fs.mkdir(skillDir, { recursive: true });
      await fs.writeFile(path.join(skillDir, 'SKILL.md'), 'test content');
    ...
    it('should preserve global tool state while reconciling a shared project root', async () => {
      ...
      const sharedSkill = path.join(sharedSkills, 'officespec-explore', 'SKILL.md');
      const globalSkill = path.join(
        testDir,
        'home',
        '.minimax',
...
`);
    ...
    // per-tool extension (gemini writes TOML), a flat ofsx-* file, and — for
    // cline — a directory that is not the tool's skillsDir at all.
    it.each([
      ['gemini', path.join('.gemini', 'commands', 'ofsx', 'explore.toml')],
      ['cursor', path.join('.cursor', 'commands', 'ofsx-explore.md')],
      ['cline', path.join('.clinerules', 'workflows', 'ofsx-explore.md')],
    ...
    it('should detect needsUpdate when a deselected workflow left a command file behind', async () => {
      ...
      const strayFile = path.join(testDir, '.claude', 'commands', 'ofsx', 'verify.md');
