...
describe('default task guidance', () => {
  it('requires a concrete verification method in each task (#345)', () => {
    ...
    expect(tasks!.instruction).toMatch(
      /a check, review,\s+observable outcome, or delivered artifact/
    );
    expect(tasks!.instruction).toMatch(
      /Put the verification in\s+that task's checkbox description/
    );
    expect(tasks!.instruction).toMatch(
      /Use a separate verification task only\s+when it checks broader outcomes that span\s+multiple tasks/
    );
    ...
    expect(numberedTasks[0]).toContain('team lead approves the fields');
    expect(numberedTasks[1]).toContain('new steps are documented');
    expect(numberedTasks[2]).toContain('everyone confirms receipt');
    expect(numberedTasks[3]).toContain('requests complete within the target time');
...
describe('propose project context', () => {
  ...
  it('applies context before exploration without granting it authority', () => {
    for (const [label, body] of proposeBodies) {
      const section = contextSection(body);
      expect(section, label).toContain('before exploring the workspace or making planning decisions');
      expect(section, label).toContain('team-provided data and constraints');
...
describe('planning code inspection (#339)', () => {
  it('inspects the project after loading instructions and dependencies, before creating or delegating artifacts', () => {
    for (const [label, body] of loopBodies) {
      ...
      const inspection = body.indexOf('**Inspect the relevant work before drafting**');
      ...
      expect(guidance, label).toContain('relevant files, docs, data, configuration, and documentation outside `openspec/`');
  ...
  it('handles separate stores, missing code, and uncertain findings without inventing facts', () => {
    for (const [label, body] of loopBodies) {
      expect(body, label).toContain('the planning home may be separate from the work');
      expect(body, label).toContain('If the target is unclear, ask');
      expect(body, label).toContain('For new initiatives, inspect the available structure and relevant documents');
  ...
  it('preserves inspection guidance through every command adapter', () => {
    for (const command of getCommandContents(['propose', 'ff'])) {
      for (const adapter of CommandAdapterRegistry.getAll()) {
        ...
        const inspection = generated.indexOf('**Inspect the relevant work before drafting**');
...
describe('propose implementation boundary', () => {
  it('makes the planning-only boundary prominent (#232, #258, #262)', () => {
    for (const [label, body] of proposeBodies) {
      ...
      expect(body, label).toContain('Do not carry out the work or edit work files');
    }
  });
  ...
  it('ends by requiring a separate apply workflow (#258, #262)', () => {
    for (const [label, body] of proposeBodies) {
      ...
      expect(body, label).toContain('Do NOT do the work');
      expect(body, label).toContain('carry out the work or edit work files');
      expect(body, label).toContain(
        'Do not start the work in the same response'
      );
      expect(body, label).toContain(
  ...
  it('preserves planning and initialization boundaries through every command adapter', () => {
    ...
    for (const adapter of CommandAdapterRegistry.getAll()) {
      ...
      expect(generated, adapter.toolId).toContain('Do NOT do the work');
      expect(generated, adapter.toolId).toContain(
        'Do not start the work in the same response'
      );
      expect(generated, adapter.toolId).toContain(
